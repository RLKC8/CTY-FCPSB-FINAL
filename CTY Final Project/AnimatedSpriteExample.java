...
public class AnimatedSpriteExample extends JPanel implements KeyListener
{
  ...
  BufferedImage basementBackground1;
  
  BufferedImage houseBackground1;
  
  BufferedImage BasementKey;
  ...
  public AnimatedSpriteExample()
  {
    ...
    BasementKey=readImage("Sprite Images/BasementKey.gif");
    
    basementBackground1 = readImage("Sprite Images/BasementBackground(1).gif");
    ...
    background = basementBackground1;
      addKeyListener(this);
      setFocusable(true);
    ...
    if(rightPressed)
      ...
      else if(leftPressed)
      {
        ...
        player.moveX(maxSpeed);
         ...
         if(background == basementBackground1)
        {
          for(int i = 0; i < obstacles.size();  i++)
          {
            if(player.intersects(obstacles.get(i)))
...
/*    else
    {
      ...
      // redraw screen
      while ((player.x>=-315)&&(player.x<=-185) && (player.y==-45)&&inBasement){
         if (!hasBasementKey){
            System.out.println("You see a key lying on the table. Do you grab it?");
      ...
      while ((player.x>=-315)&&(player.x<=-295) && (player.y<=-210)&&(player.y>=-270)&&inBasement){
         if (hasBasementKey){
            System.out.println("You use the key to open a trapdoor locking you in.");
            hasBasementKey=false;
            inBasement=false;
            inHouse=true;
            background=houseBackground1;
         }
         else{
  ...
  public void paintComponent(Graphics g)
  {
    ...
    if (!hasBasementKey&&inBasement){
      g.drawImage(BasementKey, 75-player.x, 225-player.y, null);
    }
  }
